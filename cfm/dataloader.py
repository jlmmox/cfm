...
class PairedFolderDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, root, split='train', degraded_dir='degraded', clean_dir='clean', transform=None):
        self.root = root
        self.split = split
...
class PairedFolderDataModule(pl.LightningDataModule):
    def __init__(self, root, batch_size, image_size=512, val_batch_size=None, num_workers=4,
                 seed=42, random_flip=True, pin_memory=True, persistent_workers=True, prefetch_factor=2):
        super().__init__()
        self.root = root
        self.batch_size = batch_size
        self.val_batch_size = val_batch_size if val_batch_size is not None else batch_size
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = PairedFolderDataset(self.root, split='train', transform=train_transform)
        self.val_dataset = PairedFolderDataset(self.root, split='val', transform=val_transform)
