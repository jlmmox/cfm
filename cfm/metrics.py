...
def calculate_PSNR(img1, img2):
    ...
    Args:
        img1, img2: 图像张量，范围 [0, 1]，形状 (B, C, H, W)
    ...
    psnrs = 20 * torch.log10(1.0 / torch.sqrt(mse))
    return psnrs.mean()
...
class ImageMetricTracker(nn.Module):
    ...
    def __call__(self, target, pred):
        ...
        # SSIM, PSNR, and MSE
        # 将 [-1, 1] 范围转换到 [0, 1] 范围用于计算指标
        pred_norm = pred / 2.0 + 0.5
        target_norm = target / 2.0 + 0.5
        
        # SSIM (需要 [0, 1] 范围)
        self.ssims.append(self.ssim(pred_norm, target_norm))
        
        # PSNR (需要 [0, 1] 范围)
        self.psnrs.append(calculate_PSNR(pred_norm, target_norm))
        
        # MSE (可以在原始范围 [-1, 1] 计算)
        self.mses.append(calculate_MSE(pred, target))
