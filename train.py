...
from cfm.helpers import load_model_weights
from cfm.helpers import count_params, exists
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        num_devices = args.devices if args.devices > 0 else len([torch.cuda.device(i) for i in range(torch.cuda.device_count())])

        # Use DDP only when there is more than one GPU or more than one node; otherwise avoid NCCL issues on single-GPU Windows
        strategy = None
        if num_devices > 1 or args.num_nodes > 1:
            if args.num_nodes >= 2:
    ...
    some_info = {
        ...
        'Seed': args.seed,
        # training specific (image restoration focused)
