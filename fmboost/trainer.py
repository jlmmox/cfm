...
from fmboost.metrics import calculate_PSNR
...
class TrainerFMBoost(LightningModule):
    def __init__(
        ...
        # lightweight train-time metrics (per-batch SSIM/PSNR)
        self.train_ssim = SSIM(data_range=1.)

        self.n_images_to_vis = n_images_to_vis
        self.val_epochs = 0
        self.train_metric_interval = max(0, int(train_metric_interval))

        # prefix used for logging metrics/images (e.g., "val" or "test")
        self.log_prefix = "val"
    ...
    def extract_from_batch(self, batch):
        """
        Extract high-res GT and low-res degraded inputs (and their latents).

        Training/validation should learn blur->clean, so low-res comes from
        batch["image_degraded"] when present; we fall back to downsampling
        the clean image only if degraded is missing (for backward compat).

        Returns:
            hres_ims: clean/GT images
            hres_z:   latent of GT
            lres_ims: degraded images (or downsampled clean as fallback)
            lres_z:   latent of degraded
        """

        # ground-truth clean image
        if "image" not in batch:
            raise ValueError("Batch missing clean image; dataset should provide 'image'.")
        hres_ims = batch["image"]

        # degraded input is mandatory for conditioning
        if "image_degraded" not in batch:
            raise ValueError("Batch missing image_degraded; dataset should provide degraded inputs.")
        lres_ims = batch["image_degraded"]

        # high-res latent (use precomputed if available)
        if "latent" in batch:
            hres_z = batch["latent"] * self.scale_factor
        else:
            hres_z = self.encode_first_stage(hres_ims)

        # low-res latent (use precomputed if available)
        if "latent_lowres" in batch:
            lres_z = batch["latent_lowres"] * self.scale_factor
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            self.print(f"[train] skip batch: {e}")
            return None

        # train-time image quality metrics (no grad)
        if self.train_metric_interval == 0 or self.global_step % self.train_metric_interval == 0:
            with torch.no_grad():
                hr_pred = self.predict_high_res_img(
                    lres_z=lres_z,
                    z_hr_size=hres_z.shape[-1],
                    lres_ims=lres_ims,
                    im_size=hres_ims.shape[-1],
                )
                ssim_val = self.train_ssim(hr_pred / 2 + 0.5, hres_ims / 2 + 0.5)
                psnr_val = calculate_PSNR(hr_pred / 2 + 0.5, hres_ims / 2 + 0.5)
                batch_sz = hres_ims.shape[0]
                self.log("train/ssim", ssim_val, on_step=True, on_epoch=True, batch_size=batch_sz)
                self.log("train/psnr", psnr_val, on_step=True, on_epoch=True, batch_size=batch_sz)
    ...
    def validation_step(self, batch, batch_idx):
        try:
            ...
            self.print(f"[val/test] skip batch: {e}")
            return None
        hr_pred = self.predict_high_res_img(
            lres_z=lr_z,
        ...
        # compute and log validation loss (same method as training loss)
        x_source, context, context_ca = self.get_source_and_context(
            lres_z=lr_z,
            z_size=hr_z.shape[-1],
            lres_ims=lr_ims,
        ...
        val_loss = self.forward(x_target=hr_z, x_source=x_source, context=context, context_ca=context_ca)
        self.log(f"{self.log_prefix}/loss", val_loss, on_step=False, on_epoch=True, batch_size=x_source.shape[0], sync_dist=True)
    ...
    def on_validation_epoch_end(self):
        ...
        self.log_image(out_img, self.log_prefix)
        self.vis_samples = None
        ...
        if exists(self.metric_tracker):
            ...
            for k, v in metrics.items():
                self.log(f"{self.log_prefix}/{k}", v, sync_dist=True)
